#!/usr/bin/env node
import { d, e, a, b, f, g } from './chunk-J5EMB2ZI.js';
import { resolve } from 'node:path';
import v from 'lodash';
import A from 'didyoumean';

var c=Symbol("root"),F=Symbol("skip"),_=["version","help"];var G=(e,t)=>{if(e==null||!Array.isArray(e))throw "args should be array";let n=e.slice();switch(t){case"electron":return process.defaultApp?n.slice(2):n.slice(1);case"user":return n;default:return n.slice(2)}};A.threshold=.5;var H=(e,t,n)=>{let a=G(e.args,e.from);return new Promise((s,r)=>{let{command:i,usageOptions:p,error:m}=D(t,a);if(m){r(new Error(m));return}!p.length&&i.name===c&&p.push(i.options.find(u=>u.flags.name==="help"));let l=p.filter(u=>u.operation);if(l.length){l[0].operation(),s({command:F,options:{}});return}else {let u="",g=(d,f)=>{if(d){let h=d.$data;if(!h.length&&!f.flags.value)return !f.flags.isNoFlag;if(f.flags.value)return f.flags.value.variadic?h.reduce((b,y)=>(b.push(...y.split(",").map(f.flags.value.formatter)),b),[]):f.flags.value.formatter(h[0]||f.default)}return f.default},$=i.options.reduce((d,f)=>{if(u)return d;let h=p.find(y=>y.flags.name===f.flags.name);if(!h&&f.required)return u=`required option '${f.flags.raw}' not specified`,d;let b=g(h,f);if(!f.operation){let y=f.internal||{};d[y.name||f.flags.name]=y.formatter?y.formatter(b):b;}return d},{});u?r(new Error(u)):s({command:i.name===c?null:i.name,options:$});}})},D=(e,t)=>{let n=null,a=[],s=null,r="",i=[];return v.forEach(t,(p,m)=>{if(!r){if(m===0){if(n=e[p],!n&&!p.startsWith("-")){let l=A(p,v.keys(e));r=`unknown command ${p}${l?`
(Did you mean ${l} ?)`:""}`;}else n||(n=e[c]);n&&(i=n.options.reduce((l,u)=>[...l,...u.flags.keys],[]));}if(!r){if(p.startsWith("-")){let l=n.options.find(u=>u.flags.keys.includes(p));if(!l){let u=A(p,i);r=`unknown option ${p}${u?`
...
${i}`);};var L=e=>{console.log(e.input.version);};var I={number:e=>+e,numeric:e=>+e,str:e=>`${e}`,string:e=>`${e}`,bool:e=>!!e,boolean:e=>!!e},M=e=>{let t=null,n=[],a=null,s=e.includes("--no-");v.compact(v.split(e," ").map(i=>i.replace(/,/g,""))).forEach(i=>{if(i.startsWith("-"))n.push(i);else if(a===null&&(i.startsWith("{")||i.startsWith("[")||i.startsWith("<"))){let p=i.replace(/[{[<>}\].]/g,""),m=i.includes("...");a={raw:i,variadic:m,name:p,formatter:I[p]||I.string};}});let r=n.slice().sort((i,p)=>p.length-i.length)[0];return v.isEmpty(r)||(t=v.camelCase((s?r.replace("--no-",""):r).replace(/(--?)/,""))),{isNoFlag:s,raw:e,name:t,keys:n,value:a}},x=e=>{let t=M(e.flags);return t.name?{required:!!e.required,description:`${e.description||""}`,default:e.default,flags:t,operation:e.operation,internal:e.internal}:(console.warn("invalid option",e),null)};var W=e=>{let t={},n=(s,{addVersion:r=!1,addHelp:i=!0}={})=>(t[s.name]={name:s.name,description:`${s.description||""}`,options:v.compact(v.map(s.options,x))},r&&t[s.name].options.unshift(x({flags:"-v, --version",description:"output the current version",operation:()=>L(a)})),i&&t[s.name].options.push(x({flags:"-h, --help",description:"display help for command",operation:()=>R(t,a,t[s.name])})),a),a={commands:t,input:e,addCommand:n,execute:s=>H(s,t)};return n({name:c,options:[]},{addVersion:!1,addHelp:!1}),v.forEach(e.options,s=>{let r=x(s);if(r){if(_.includes(r.name)){console.warn("reserved option",r.name);return}t[c].options.push(r);}}),t[c].options.unshift(x({flags:"-v, --version",description:"output the current version",operation:()=>L(a)})),t[c].options.push(x({flags:"-h, --help",description:"display help for command",operation:()=>R(t,a,t[c])})),v.forEach(e.commands,n),a};var o=new d({}),w=new e({}),V=W({name:a.name,alias:"sta",version:a.version,description:`Generate api via swagger scheme.
